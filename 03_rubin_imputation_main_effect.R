################################################################################
# Purpose:
# This script processes 100 imputed datasets of daily road traffic deaths using
# Rubin's rules to obtain pooled estimates of the temperature-mortality
# relationship in 272 Latin American cities.
#
# For each imputed dataset:
# - A distributed lag non-linear model (DLNM) is fit.
# - The model estimates temperature effects on daily road deaths.
# - Coefficients and variance-covariance matrices are stored.
#
# Rubin’s rule is then used to combine estimates across imputations.
# The script finally reconstructs pooled DLNM predictions and generates plots.
#
# Input:  100 CSV files containing imputed daily death counts + covariates.
# Output: Pooled coefficients and variance matrices (.rds) and final DLNM plots.
#
...
col='gray'
  ...
  mtext("b", side = 3, adj = -.35, line = 0, cex = 1)  # Label 'b'
  ...
  hist(data[['tmp_pw_percentile']], breaks = 100, probability = TRUE, col = "grey60", main = '', xlab = "Temperature (%tile)", ylab = "Probability Density",  ylim=c(0,0.15))
  abline(v = c(quan01,quan25,medT,quan75,quan99), lty = 4, col = "gray")
  mtext("c", side = 3, adj = -.35, line = 1, cex = 1) # Label 'c'
  
  hist(data[['L1ADtemp_pw']], breaks = 100, probability = TRUE, col = "grey60", main = '', xlab = "Temperature (°C)", ylab = "Probability Density", ylim=c(0,0.15))
  abline(v = c(quan01_celcius,quan25_celcius,medT_celcius,quan75_celcius,quan99_celcius), lty = 4, col = "gray")
  mtext("d", side = 3, adj = -.35, line = 1, cex = 1)  # Label 'd'
  ...
  col <- 'gray'
    ...
    # Plot a
    plot(pred_road_reconstruct_pooled, "overall", lag = 0, cumul = TRUE, ylim = c(0.9, 1.4), 
         ylab = "RR", col = col, lwd = 1.5, ci = 'area', lty = 1,
         xlab = "Temperature (%tile)", ci.arg = list(col = alpha(col, 0.3)))
    ind2 <- pred_road_reconstruct_pooled$predvar >= quan01 & pred_road_reconstruct_pooled$predvar <= quan99
    lines(pred_road_reconstruct_pooled$predvar[ind2], pred_road_reconstruct_pooled$allRRfit[ind2], 
          col = 'firebrick3', lwd = 1.5)
    abline(v = c(quan01, quan25, medT, quan75, quan99), lty = 4, col = "gray")
    mtext("a", side = 3, adj = -0.3, line = 0, cex = 1)  # Label 'a'
    
    # Plot b
    plot(pred_road_reconstruct_celcius_pooled, "overall", lag = 0, cumul = TRUE, ylim = c(0.9, 1.4),
         ylab = "RR", col = col, lwd = 1.5, ci = 'area', lty = 1,
         xlab = "Temperature (°C)", ci.arg = list(col = alpha(col, 0.3)))
    ind2 <- pred_road_reconstruct_celcius_pooled$predvar >= quan01_celcius & pred_road_reconstruct_celcius_pooled$predvar <= quan99_celcius
    lines(pred_road_reconstruct_celcius_pooled$predvar[ind2], pred_road_reconstruct_celcius_pooled$allRRfit[ind2], 
          col = 'firebrick3', lwd = 1.5)
    abline(v = c(quan01_celcius, quan25_celcius, medT_celcius, quan75_celcius, quan99_celcius), lty = 4, col = "gray")
    mtext("b", side = 3, adj = -0.3, line = 0, cex = 1)  # Label 'b'
    
    # Plot c (Histogram for Temperature Percentile)
    hist(data[['tmp_pw_percentile']], breaks = 100, probability = TRUE, col = "grey60", main = '', 
         xlab = "Temperature (%tile)", ylab = "Probability Density", ylim = c(0, 0.15))
    abline(v = c(quan01, quan25, medT, quan75, quan99), lty = 4, col = "gray")
    mtext("c", side = 3, adj = -0.3, line = 1, cex = 1)  # Label 'c'
    
    # Plot d (Histogram for Temperature in °C)
    hist(data[['L1ADtemp_pw']], breaks = 100, probability = TRUE, col = "grey60", main = '',
         xlab = "Temperature (°C)", ylab = "Probability Density", ylim = c(0, 0.15))
    abline(v = c(quan01_celcius, quan25_celcius, medT_celcius, quan75_celcius, quan99_celcius), lty = 4, col = "gray")
