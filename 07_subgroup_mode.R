################################################################################
# Purpose:
# This script conducts subgroup analysis by mode of transport (vehicle, motorcycle, 
# bicycle, pedestrian) using both the median-imputation dataset and pooled estimates
# across 100 imputations via Rubin's rule.
#
# For the median-imputation dataset (non-imputed), generalized nonlinear models (GNM)
# are fitted using distributed lag non-linear models (DLNM). Models are stratified 
# by city-date-weekday combinations. Estimates are extracted using `crosspred`.
#
# For imputed datasets, this script:
# - Loops through road1.csv to road100.csv
# - Extracts model coefficients and variance-covariance matrices by mode
# - Applies Rubin's rule to pool estimates
# - Saves pooled results to RDS files
# - Reconstructs predicted cumulative effects using pooled estimates
# - Plots pooled vs. non-imputed results
#
# Input: Median and 100-imputed datasets of daily road deaths by mode
# Output: Pooled coefficients/variances and comparative plots
#
...
pred_pooled_list[['motorcycle']]$allRRfit[991]
pred_pooled_list[['motorcycle']]$allRRlow[991]
pred_pooled_list[['motorcycle']]$allRRhigh[991]

pred_pooled_list[['bicycle']]$allRRfit[991]
pred_pooled_list[['bicycle']]$allRRlow[991]
pred_pooled_list[['bicycle']]$allRRhigh[991]


pred_pooled_list[['vehicle']]$allRRfit[991]
pred_pooled_list[['vehicle']]$allRRlow[991]
pred_pooled_list[['vehicle']]$allRRhigh[991]

pred_pooled_list[['ped']]$allRRfit[991]
pred_pooled_list[['ped']]$allRRlow[991]
pred_pooled_list[['ped']]$allRRhigh[991]
