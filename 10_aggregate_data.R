...
# Path to data
path_1 <- "//files.drexel.edu/colleges/SOPH/Shared/UHC/Projects/Wellcome_Trust/Manuscripts/MS252_Hsu/Data Build_20240821/Derived Data/"
path_1 <- "C:/Users/dpw48/Documents/MS252_Hsu/Data Build_20240917/Derived Data/"
...
custom_summary_function <- function(df, i = 1) {
  # Names of imputed columns to extract
  road       <- paste0("road", i)
  vehicle    <- paste0("vehicle", i)
  motorcycle <- paste0("motorcycle", i)
  bicycle    <- paste0("bicycle", i)
  ped        <- paste0("ped", i)
  
  imputed_names <- c(road = road,
                     vehicle = vehicle,
  ...
  df %>% 
    summarize(
      # Total road deaths by case category
      male_deaths    = sum(.data[[road]]*(male == 1)),
      female_deaths  = sum(.data[[road]]*(male == 0)),
      deaths_under_9 = sum(.data[[road]]*(age_cat == "<=9")),
      deaths_10_19   = sum(.data[[road]]*(age_cat == "10-19")),
      deaths_20_34   = sum(.data[[road]]*(age_cat == "20-34")),
      deaths_35_64   = sum(.data[[road]]*(age_cat == "35-64")),
      deaths_65_plus = sum(.data[[road]]*(age_cat == "65+")),
      # Country, city, date time information (for stratification)
      across(c(country,
               city_size,
               salid1,
               year_month,
               dow), first),
      # Total road deaths by mode
      across(c(deaths,
               imputed_names), sum),
...
path_2 <- "C:/Users/dpw48/Documents/MS252_Hsu/2024_01_03/"
...
path_3 <- "C:/Users/dpw48/Documents/MS252_Hsu/2024_02_27/"
...
# Each file is an imputation
# Each row represents a city-year-month-day
path_write <- "C:/Users/dpw48/git/MS252/Data/"
...
walk(1:n_imp, \(i) {
  ...
  write_csv(df_i, paste0(path_write, "final_temp_cluster_subgroup_", i, ".csv"))
})
