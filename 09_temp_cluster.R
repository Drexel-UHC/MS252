################################################################################
# Purpose:
# This script performs DLNM-based subgroup analysis by temperature clusters (k=6)
# across 100 imputed datasets. Each cluster is evaluated for the pooled association
# between temperature percentile and road traffic deaths. Rubin's rule is applied 
# to pool coefficients and variance estimates. Predictions are reconstructed for
# each cluster for plotting and interpretation.
#
# Input: 100 imputed road files, temp cluster assignment, and modifiers
# Output: Pooled coefficient and VCOV RDS files per cluster, reconstructed prediction objects
#
...
for (i in 1:num_clusters) {
  ...
  pred_reconstruct_pooled[[i]] <- crosspred(cbt, model.link = "log",
                                            ...
                                            cum = TRUE, at = seq(minT, maxT, by = 0.1), cen = quan01)
}
